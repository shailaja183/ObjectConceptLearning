...
class Dogs(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        self.idx2label = {}
        for en, af in enumerate(os.listdir(self.annotations_folder)):
            target = "-".join(af.split("-")[1:])
            self.idx2label[en] = target.replace("_", " ").lower()
    ...
    def __getitem__(self, index):
        ...
        return image, self.idx2label[target]
...
if __name__ == '__main__':
    train_dataset = Dogs('./data/dogs', train=True, download=True)
    test_dataset = Dogs('./data/dogs', train=False, download=False)